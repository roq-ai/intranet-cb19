...
function DashboardPage() {
  ...
  const rewardStat = useSWR<PaginatedInterface<RewardInterface>>(
    () => `/rewards`,
    () => getRewards({ limit: 1, offset: 0 }),
  );

  const newsStat = useSWR<PaginatedInterface<NewsInterface>>(
    () => `/news`,
    () => getNews({ limit: 1, offset: 0 }),
  );

  const vlogStat = useSWR<PaginatedInterface<VlogInterface>>(
    () => `/vlogs`,
    () => getVlogs({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; isLoading: boolean }[] = [
    {
      title: 'Rewards',
      result: rewardStat?.data?.totalCount,
      isLoading: rewardStat.isLoading,
    },
    {
      title: 'News',
      result: newsStat?.data?.totalCount,
      isLoading: newsStat.isLoading,
    },
    {
      title: 'Vlogs',
      result: vlogStat?.data?.totalCount,
      isLoading: vlogStat.isLoading,
    },
  ];
