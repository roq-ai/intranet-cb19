...
type ColumnType = ColumnDef<VlogInterface, unknown>;

interface VlogListPageProps {
...
export function VlogListPage(props: VlogListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getVlogs({
        ...
        order: params.order,
        searchTermKeys: ['title.contains', 'url.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<VlogInterface>>(
    () => `/vlogs?params=${JSON.stringify(params)}`,
    fetcher,
  );
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/vlogs/view/${record.id}`} passHref legacyBehavior>
                <Button
